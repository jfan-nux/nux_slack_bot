...
, avg(order_freq_numerator) as order_frequency
, avg(new_cx) as new_cx_rate
...
from checkout c 
left join MAU m 
on m.tag = c.tag AND m.segments = c.segments
)

SELECT r1.*
        , r1.order_rate / NULLIF(r2.order_rate,0) - 1 AS Lift_order_rate
        , r1.order_frequency / NULLIF(r2.order_frequency,0) - 1 AS Lift_order_frequency
        ...
        , r2.order_frequency AS control_order_frequency
