...
WITH experiment AS (
    ...
    WHERE 1=1
    AND a.experiment_name = 'should_enable_percentage_eta_threshold_job'
    AND convert_timezone('UTC', 'America/Los_Angeles', a.exposure_time)::date >= '2025-09-10'
    AND experiment_version = 5
    AND tag <> 'overridden'
    GROUP BY all
...
, device_level_temp AS (
  ...
  WHERE scd_start_date >= '2025-09-10'
